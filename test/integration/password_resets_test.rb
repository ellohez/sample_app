...
class PasswordResetForm < PasswordResets
  def setup
    ...
    # NOTE: assigns is not included by default in Rails > 5 but is included in a gem called rails-controller-testing
    # it automatically looks for an instance variable with name corresponding to the given symbol (i.e. @user for @user)
    # lets us access instance variables in the corresponding action (Users controller create action in this case)
...
class ExpiredToken < PasswordResets
  def setup
    ...
    post password_resets_path params: { password_reset: { email: @user.email } }
    @reset_user = assigns(:user)
    # Expire the password reset token by hand
    @reset_user.update_attribute(:reset_sent_at, 3.hours.ago)
    # Attempt to update the password with an expired token
    patch password_reset_path(@reset_user.reset_token),
          params: { email: @reset_user.email,
